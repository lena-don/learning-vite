...
2. Отдаёт `index.html` почти как есть, но инжектит (inject — принудительная передача данных/зависимостей/кода) `<script src="/@vite/client">` — клиент HMR (Горячая замена модулей Hot Module Replacement — позволяет обновлять файлы в запущенном приложении во время работы без необходимости перезагружать всю страницу целиком).
...
3. Транспиляция (перевод исходного кода программы с одного языка программирования на другой язык с похожим уровнем абстракции), минификация (процесс удаления из исходного кода лишних символов без изменения его работы, чтобы уменьшить размер файлов и ускорить загрузку сайтов), tree-shaking («встряхивание дерева» — метод оптимизации кода, который удаляет неиспользуемый «мертвый» код из итогового файла при сборке приложения), code splitting (метод оптимизации веб-приложений, при котором общий файл со скриптами (бандл) разбивается на более мелкие независимые части (чанки), загружаемые по требованию).
